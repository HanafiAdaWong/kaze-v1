...
import { getSamehadakuDetail } from '../services/api'
import Loader from '../components/Loader'

function SamehadakuDetail() {
    ...
    useEffect(() => {
        ...
        async function fetchDetail() {
            ...
            setError(null)
            try {
                const data = await getSamehadakuDetail(slug)
                if (!cancelled) {
    ...
    if (loading) {
        return (
            <div style={{ paddingTop: 'var(--navbar-height)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Loader text="Memuat detail Samehadaku..." />
            </div>
        )
    }
    ...
    const synopsis = detail.synopsis?.paragraphs?.[0] || ''
    const type = detail.type || 'Anime'
    const status = detail.status || ''
    const rating = detail.score?.value || ''
                    ...
                    <div className="watch-detail-hero__content">
                        ...
                        <div className="watch-detail-hero__info">
                            ...
                            {detail.genreList?.length > 0 && (
                                <div className="detail__genres" style={{ marginTop: '12px' }}>
                                    {detail.genreList.map(g => (
                                        <span key={g.genreId} className="badge" style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', background: 'rgba(255,255,255,0.1)' }}>
                                            <Tag size={12} /> {g.title}
                                        </span>
                                    ))}
                                </div>
                            ...
                            <div style={{ marginTop: '20px', display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                                {episodes.length > 0 && (
                                    <Link
                                        to={`/samehadaku/episode/${episodes[episodes.length - 1].episodeId}`}
                ...
                {episodes.length === 0 ? (
                    ...
                    <div className="episode-grid">
                        {[...episodes].reverse().map((ep, i) => (
                            <Link
                                key={ep.episodeId}
                                to={`/samehadaku/episode/${ep.episodeId}`}
                            ...
                            >
                                ...
                                <div className="episode-card__info">
                                    <span className="episode-card__title">
                                        {ep.title ? (isNaN(ep.title) ? String(ep.title).replace(/Episode/gi, '').trim() : ep.title) : episodes.length - i}
                                    </span>
                                </div>
                            </Link>
                        ))}
...
export default SamehadakuDetail
